...
export class EditEmployeeComponent implements OnInit {
  ...
  passwordPatternList = [
    ...
    {viewValue: 'At least one uppercase letter, one lowercase letter, one number and one special character', value: 7}
  ]
  previousPassword = [
    ...
    try {
      if(this.accessControlForm.value.accessControlSystem) {
        this.accessControlApiService.getFullSystemGatesGroupsList(this.accessControlForm.value.accessControlSystem).subscribe((res: any) => {
          ...
          this.accessControlGroupsList.sort((a,b) => a.name.localeCompare(b.name));
  ...
  onSystemChange(newSystem) {
    ...
    if(newSystem.value == '') 
      this.accessControlForm.patchValue({
        accessControlLevel: ACCESS_CONTROL_LEVEL.NONE
      });
    this.getSystemGates();
  ...
  async getDocumentsList() {
    try {
      ...
      if(res.result) {
        ...
        if(this.searchD == "") this.searchD = this.documentList[0].id;
      }
    }
    catch(e) {}
  }
  ...
  async getUserDocumentList() {
    try {
      ...
      if(res) {
        ...
        if(this.documentUploaded && this.documentUploaded.documentId == this.searchD) {
...
openAddAlternativeFieldDialog(){
    const dialogRef = this.dialog.open(AddEditDisputePopupComponent,{
      data: {placeholder: 'Alternative Area Name', heading: 'Add Alternative Field',validation:'Enter your alternative area name'}
    });
}
    ...
    if(step == 4){
      ...
      if(this.badgeModified) {
        ...
        const dialogRef = this.dialog.open(YesNoPopupComponent,{
          data: {title: this.translate.instant("ACCESS_CONTROL.RegenerateBadge"), message1: message1, message2: this.translate.instant("ACCESS_CONTROL.Sure"), icon: "qr_code"}
        });
    ...
    if(step == 5){
      try {
        if(this.documentUploaded) {
          let renew = false;
          if(this.userAccount.accessControlLevel=='SINGLE_PASSAGES') {
            let userGates = this.accessControlGatesListFull.filter(gate => this.finalValueForm.value.accessControlPassagesId.find(userGate => gate.id == userGate));
            for(let gate of userGates) {
              if(gate.documentId == this.documentUploaded.documentId) renew = true;
            }
          }
          else if(this.userAccount.accessControlLevel=='GROUP') {
            let userGroups = this.accessControlGroupsList.filter(group => this.finalValueForm.value.accessControlGroupsId.find(userGroup => group.id == userGroup));
            let userGates = new Array();
            for(let group of userGroups) {
              let groupGates = this.accessControlGatesListFull.filter(gate => group.gatesId.find(groupGate => groupGate == gate.id));
              for(let groupGate of groupGates) {
                if(!userGates.find(gate => gate.id == groupGate.id)) userGates.push(groupGate);
              }
              for(let gate of userGates) {
                if(gate.documentId == this.documentUploaded.documentId) renew = true;
              }
...
editPeopleView(id){
  this.adminUserManagementService.getPeopleForEdit(id).then((response) => {
    ...
    this.originalPerson = responseData.user;
    try {
    ...
    this.finalValueForm.patchValue({
      ...
      city: values.city,
      cap: values.cap,
      addressDomicilio: values.addressDomicilio,
      cityDomicilio: values.cityDomicilio,
      capDomicilio: values.capDomicilio,
      provDomicilio: values.provDomicilio,
      residenceAsDomicilio: values.residenceAsDomicilio,
      email: values.email,
      phone: values.phone,
    ...
    this.checkDomicileEnableDisable();
  });
  ...
  editUserDocument(document: any) {
    ...
    dialogRef.afterClosed().subscribe(async (result) => {
      if(result) {
        ...
        Swal.fire(
          '',
          this.translate.instant('DOCUMENTS.DocumentModified'),
          'success'
        );
        await this.getUserDocumentList();
      }
  ...
  async downloadUserDocument(doc: any) {
    try {
      ...
      if(res) {
        ...
        Swal.fire(
          '',
          this.translate.instant('DOCUMENTS.DocumentDownloaded'),
          'success'
        );
      }
      else {
  ...
  async deleteUserDocument(document: any) {
    ...
    if(doc) {
      ...
      if(res) {
        Swal.fire(
          '',
          this.translate.instant('DOCUMENTS.DocumentDeleted'),
          'success'
        );
        await this.getUserDocumentList();
      }
  ...
  async onUploadDoc() {
    const dialogRef = this.dialog.open(UploadUserDocumentPopupComponent,{
      data: {personId: this.id, documentId: this.searchD}
    });
    dialogRef.afterClosed().subscribe(async (result) => {
      if(result) {
        this.documentUploaded = result;
        await this.getDocumentsList();
        await this.onDocTabChange({index: this.documentList.map(e => e.id).indexOf(result.documentId)});
      }
    });
  }
  
  async onDocTabChange(event: any) {
    this.docTabIndex = event.index;
    this.searchD = this.documentList[this.docTabIndex].id;
    await this.getUserDocumentList();
  }

  async saveAndExit() {
    let findAreaUID = this.sortedAreas.find((b) => {
      return b.name === this.settingsForm.value.area;
    });

    let managerName = "";
    if(this.settingsForm.value.manager) {
      let manager = this.people.find(person => person.id == this.settingsForm.value.manager);
      if(manager) managerName = manager.name + " " + manager.surname;
    }
    let generalRawValues = this.generalDetailsForm.getRawValue();
    this.finalValueForm.patchValue({
      name: this.generalDetailsForm.value.name,
      surname: this.generalDetailsForm.value.surname,
      cf: this.generalDetailsForm.value.fiscalCode,
      gender: this.generalDetailsForm.value.gender,
      birthDay: this.generalDetailsForm.value.birthday,
      birthPlace: this.generalDetailsForm.value.birthPlace,
      birthProv: this.generalDetailsForm.value.birthProv,
      companyId: this.userAccount.companyId,
      scope: this.scope?.name,
      address: this.generalDetailsForm.value.address,
      city: this.generalDetailsForm.value.city,
      cap: this.generalDetailsForm.value.cap,
      prov: this.generalDetailsForm.value.prov,
      addressDomicilio: generalRawValues.addressDomicilio,
      cityDomicilio: generalRawValues.cityDomicilio,
      capDomicilio: generalRawValues.capDomicilio,
      provDomicilio: generalRawValues.provDomicilio,
      residenceAsDomicilio: this.generalDetailsForm.value.residenceAsDomicilio,
      
      phone: this.contactInformationForm.value.phone,
      email: this.contactInformationForm.value.email,
      personalPhone: this.contactInformationForm.value.personalPhone,
      personalEmail: this.contactInformationForm.value.personalEmail,
      ...
      identificationCode: this.settingsForm.value.codeIdentity,
      partTimePercentage: this.settingsForm.value.partTime,
      managerId: this.settingsForm.value.manager,
      ...
      salary: this.economicsFieldsForm.value.salary,
      mbo: this.economicsFieldsForm.value.mbo,
      bonus: this.economicsFieldsForm.value.bonus,
      contractLevel: this.economicsFieldsForm.value.contractLevel,
      ccnl: this.economicsFieldsForm.value.ccnl,
      patrocinante: this.economicsFieldsForm.value.patrocinante,
    ...
    if(this.documentUploaded) {
      if(this.finalValueForm.value.accessControlLevel=='SINGLE_PASSAGES') {
        let userGates = this.accessControlGatesListFull.filter(gate => this.finalValueForm.value.accessControlPassagesId.find(userGate => gate.id == userGate));
        for(let gate of userGates) {
          if(gate.documentId == this.documentUploaded.documentId) renew = true;
        }
      }
      else if(this.finalValueForm.value.accessControlLevel=='GROUP') {
        let userGroups = this.accessControlGroupsList.filter(group => this.finalValueForm.value.accessControlGroupsId.find(userGroup => group.id == userGroup));
        let userGates = new Array();
        for(let group of userGroups) {
          let groupGates = this.accessControlGatesListFull.filter(gate => group.gatesId.find(groupGate => groupGate == gate.id));
    ...
    if(renew || this.badgeModified) {
      if(this.finalValueForm.value.accessControlLevel == ACCESS_CONTROL_LEVEL.NONE || this.finalValueForm.value.accessControlSystem=='') { 
        if(this.originalPerson.accessControlLevel == ACCESS_CONTROL_LEVEL.GROUP || this.originalPerson.accessControlLevel == ACCESS_CONTROL_LEVEL.SINGLE_PASSAGES ||
        this.originalPerson.accessControlLevel == ACCESS_CONTROL_LEVEL.PASSEPARTOUT) {
          let message1 = this.translate.instant("BlockPersonAccessControl1") + this.finalValueForm.value.surname + " " + this.finalValueForm.value.name + this.translate.instant("BlockPersonAccessControl2");
          const dialogRef = this.dialog.open(YesNoPopupComponent,{
            data: {title: this.translate.instant("BlockBadge"), message1: message1, message2: this.translate.instant("ACCESS_CONTROL.Sure"), icon: "qr_code"}
          });
          dialogRef.afterClosed().subscribe(async (result) => {
            if(result) {
              await this.adminUserManagementService.addOrUpdatePerson(this.finalValueForm.value);
              ...
              if(res.result) {
                Swal.fire(
                  '',
                  this.translate.instant('PersonUpdatedAndBadgeBlocked'),
                  'success'
                );
              }
              this.router.navigate(['/people']);
        ...
        dialogRef.afterClosed().subscribe(async (result) => {
          if(result) {
            ...
            if(res.result) {
              Swal.fire(
                '',
                this.translate.instant('PersonUpdatedAndBadgeRegenerated'),
                'success'
              );
            }
            this.router.navigate(['/people']);
  ...
  checkCF() {
    try {
        let cf = new CodiceFiscale(this.generalDetailsForm.value.fiscalCode);
        let gender = "";
  ...
  changeDomicileResidence(event: any) {
    if(event.checked) {
      this.generalDetailsForm.patchValue({
        addressDomicilio: this.generalDetailsForm.value.address,
        cityDomicilio: this.generalDetailsForm.value.city,
        provDomicilio: this.generalDetailsForm.value.prov,
        capDomicilio: this.generalDetailsForm.value.cap,
      })
      this.checkDomicileEnableDisable();
    }
  }

  checkDomicileEnableDisable() {
    if(this.generalDetailsForm.value.residenceAsDomicilio) {
      this.generalDetailsForm.controls["addressDomicilio"].disable();
      this.generalDetailsForm.controls["cityDomicilio"].disable();
      this.generalDetailsForm.controls["capDomicilio"].disable();
      this.generalDetailsForm.controls["provDomicilio"].disable();
    }
    else {
      this.generalDetailsForm.controls["addressDomicilio"].enable();
      this.generalDetailsForm.controls["cityDomicilio"].enable();
      this.generalDetailsForm.controls["capDomicilio"].enable();
      this.generalDetailsForm.controls["provDomicilio"].enable();
    }
  }

  clearResignationDate() {
    this.settingsForm.patchValue({
