...
@Component({
  selector: 'app-add-edit-supplier-privacy-policy',
  templateUrl: './add-edit-supplier-privacy-policy.component.html',
  styleUrls: ['./add-edit-supplier-privacy-policy.component.scss']
})
export class AddEditSupplierPrivacyPolicyComponent implements OnInit {
  addSupplierPrivacyForm:FormGroup;
  dataSource:any;
  cmsObject:any;
  ...
  constructor(
    ...
    this.addSupplierPrivacyForm = this._formBuilder.group({
      id:[''],
      descriptionEN: [''],
      descriptionIT: [''],
      descriptionES: [''],
  ...
  ngOnInit(): void {
    this.getSupplierPrivacyPolicyCMS();
  }

  getSupplierPrivacyPolicyCMS() {
    let typeName = 'procurement_privacy_policy';
    this.helper.toggleLoaderVisibility(true);
    this.helper.getCMSDetails({type : typeName}).subscribe((data:any) =>{
      this.privacyDataObj = data.data;
      this.addSupplierPrivacyForm.patchValue({
  ...
  async addPrivacy(): Promise<void> {
    this.helper.toggleLoaderVisibility(true);
    const res: any = await this.cmsManagementService.saveCMSList(this.addSupplierPrivacyForm.value)
    if(this.addSupplierPrivacyForm.valid){
      this.helper.toggleLoaderVisibility(false);
