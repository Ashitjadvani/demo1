...
export class RecruitingApplicationManagementService {
    ...
    async getCandidateUniversities(): Promise<any> {
        return this.apiService.get<any>(this.apiService.API.BE.RECRUITING_CANDIDATE_UNIVERSITIES).toPromise();
    }

    async getCandidateAreas(): Promise<any> {
        return this.apiService.get<any>(this.apiService.API.BE.RECRUITING_CANDIDATE_AREAS).toPromise();
    }
}
